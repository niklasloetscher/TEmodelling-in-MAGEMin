...
const IDEAL_CHARGES = Dict{String, Int}(
	"ol" => 2,
	"cpx" => 2,
	"opx" => 2,
	"amp" => 2,
...
function _charge_correction_sign(mineral::String, charge::Int, closest_charge::Int) :: Float64
    if haskey(IDEAL_CHARGES, mineral)
        ideal = IDEAL_CHARGES[mineral]
        return abs(charge - ideal) < abs(closest_charge - ideal) ? 1.0 : -1.0
    else
        return -1.0
    end
end
        ...
        for (i, el) in enumerate(elements)
            ...
            if isEu
                ...
                if haskey(LSM_AUTHORS, (mineral, 2))
                    ...
                    sgnEu = _charge_correction_sign(mineral, 2, closest_chargeEu)
                    D_Eu2 = _LSM_D(mineral, el, 2, authorsEu, params) * exp(sgnEu * A * dchargeEu^2 / (R * params[:T])) # Adjust for charge mismatch using Wood & Blundy (2014), direction set by IDEAL_CHARGES
                end
                # Compute Eu D
                ...
                if dcharge == 0
                    ...
                    sgn = _charge_correction_sign(mineral, charge, closest_charge)
                    D_dict[el] = _LSM_D(mineral, el, charge, authors, params) * exp(sgn * A * dcharge^2 / (R * params[:T])) # Adjust for charge mismatch using Wood & Blundy (2014), direction set by IDEAL_CHARGES
                end
            end
